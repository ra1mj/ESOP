...
当前工作区另有 `crates/esop-lifecycle-guard/`，提供独立的固定门槛、motion permit 生命周期守卫和固定容量状态转换审计；`crates/esop-ebpf-agent/` 提供固定证据 ABI、问题相关器、健康心跳和 eBPF 能力预检结果模型。它们只负责普通控制域的 fail-closed 策略和运行时观测，不替代 STO、FSoE、安全 PLC 或认证安全通道。
