...
本地质量门统一由 `make ci` 执行，包含 workspace 测试、Clippy、release 构建、`aarch64-unknown-none` 核心检查、BPF C 语法检查和 Zenoh feature 编译检查；`make setup-rust` 安装交叉检查目标，`make test-hil` 只运行 Linux 模拟端口 HIL 测试，`make test-zenoh` 启动 loopback `zenohd` 验证真实路由，`make bpf` 执行需要 clang 和内核 BTF 的完整 CO-RE 编译。
