...
当前 `crates/esop-cfggen/` 已把上述注册层用于宿主机产品编译：显式 ESI identity/PDO 选择、稳定 Rx-then-Tx offset、每 Domain 的 LWR/LRD、expected WKC、多速率 schedule、CiA 402 对象/缩放/限幅和 ProcBuf ABI v6 布局会在发布前统一验证。`crates/esop-product-config/` 消费生成的 Rust 静态数据，在激活期通过相同注册/校验 API 重建并冻结计划；配置 hash、ProcBuf、精确从站记录、Domain 证据或轴映射任一不一致均拒绝。它还能按从站生成 assignment-clear/mapping/assignment-publish 启动计划；核心控制器对每个写值追加精确 SDO upload 回读，长度或内容不一致即故障。`ScheduledPdoConfiguration` 已把计划执行接入统一生产调度、邮箱/DC/共享 RX、请求重建/超时和 Configuration 生命周期门。可选 Startup PREOP 屏障会等待全部期望从站，再按冻结的 PDO/Mapping/DC 必需集合运行；只有所有真实控制器 Complete 才复用已验证拓扑继续 SAFEOP/OP，且配置期间 Topology 始终由报告中的 Startup phase 收紧。调用方仍须启动逐从站配置任务并提供 MailboxConfig/映射/DC 描述；真实响应真实性、自动多从站批次、完整周期 WKC、从站互操作或 HIL 仍未完成，该软件证据不能替代产品资格。
