...
从站身份。产品运行时现可用 position-keyed `ProductMailboxBinding` 事务式生成完整
固定容量 PDO 批次；`PdoConfigBatch` 为每个 job 分配独立 generation，并由生产
调度器在 PREOP 屏障内自动推进。第一项完成不会放行 Startup，空 job 仅作有界跳过，
故障保留原 index/station 并要求显式重启；生产报告公开批 phase/index/count/current
station。MailboxConfig/SM-FMMU/DC 自动发现、真实响应来源、物理从站互操作和 HIL
