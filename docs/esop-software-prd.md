...
R2 当前进展：固定容量 `ScheduledDomainBank` 已拥有多 Domain 共享 RX，`ScheduledAuxiliaryOutputs` 按**下一次 RX 周期**的冻结 due mask 提前提交辅助输出，邮箱/DC 服务周期可直接接入 MLG 非总线事实，生命周期入口会在 State/事件发布后完成最终 deadline 观测和补救。`ScheduledProcessInputs` 可在启动、恢复或其他明确需要预 RX 发送的阶段完成首次 priming；`ScheduledProductionCycleOwner` 随后以 `PrimingRequired → ReceiveArmed → OutputPending → ReceiveArmed` 固定状态交接上一轮生命周期输出，核对周期、generation、RX deadline 和实际完成报告，拒绝重复 priming，并把完整过程 handoff、最终 deadline、State 与事件发布合并为任务释放证据。`run_dc_and_control_cycle` 现将启动、映射、DC 配置等调用方持有的非邮箱控制请求纳入同一 DC/共享 RX 阶段，`Prepared` 只发送一次、`InFlight` 只等待完成或超时，终态仍由原服务 FSM 消费；生命周期入口按显式 Platform/Configuration/Topology/Drive 所有权投影失败。仍缺一个统一选择邮箱与各非邮箱服务、管理恢复/重建并覆盖完整任务入口的生产调度器；受控 Hold/Ramp、目标硬件 WCET 与实物 HIL 验收也未完成，上表 R2 出口条件仍未满足。
