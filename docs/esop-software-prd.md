...
| R2：伺服与实时资格 | CoE、DC、ProcBuf、MLG、CiA 402 单轴/双轴和 IO、故障策略。 | 两种驱动 + IO 完成 HIL；Q1/Q2、无动态分配、MLG 状态机和故障矩阵证据通过。当前已具备跨层质量门投影、LifecycleSnapshot、双轴独立许可、配置停止动作、新 permit epoch 恢复、ProcBuf v3 完整生命周期摘要与原始周期质量位图、固定转换历史发布、证据绑定能力清单和构建/性能报告校验测试；实物 HIL 证据仍待补齐。 |
