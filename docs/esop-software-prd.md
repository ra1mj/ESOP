...
| R2：伺服与实时资格 | CoE、DC、ProcBuf、MLG、CiA 402 单轴/双轴和 IO、故障策略。 | 两种驱动 + IO 完成 HIL；Q1/Q2、无动态分配、MLG 状态机和故障矩阵证据通过。当前已具备跨层质量门投影、LifecycleSnapshot、双轴独立许可、配置停止动作、新 permit epoch 恢复、ProcBuf v4 生命周期摘要、逐轴请求/发出控制字/反馈证明位及原始周期质量位图、固定转换历史发布、停机超时的逐轴升级事件生成和满环重试接口、证据绑定能力清单和构建/性能报告校验测试；生产周期所有者接线、受控 Hold/Ramp、全量生命周期事件发布与实物 HIL 证据仍待补齐。 |
