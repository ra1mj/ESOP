...
| FR-028 | P0 | 每个关节应暴露请求模式、目标位置/速度/转矩、实际值、使能、状态字、故障、时间戳和质量。 | 生成 layout、实物 PDO offset 与 SI 单位/缩放定义交叉验证。当前软件路径已用冻结每轴策略完成 SI 命令到 raw CSP/CSV/CST PDO 的确定性换算、机械/命令上限校验、使能边沿实际值保持和事务提交；生成配置与实物定义的交叉验证仍是产品验收项。 |
| FR-029 | P1 | IPC 应支持共享内存、RPMsg 或 Unix domain socket 的受控实现，并传递版本、序号、时间戳、质量与掉线状态。当前已实现宿主机文件系统 Unix datagram：固定容量版本化帧、显式小端布局、CRC、精确 peer path 准入、非阻塞收发、按 robot/layout/source/boot/sequence/time 检测离线/重连/重启，以及 ProcBuf State/Event 到 Protobuf 帧和 MotionCommand target 经结构验证、策略准入、ProcBuf v5 Command page 发布到 RT permit 重建的共享适配；下游 RT 已把该 Command page 与当前 MLG permit 精确绑定并接入 CiA 402 活动帧。 | 每种声明支持的 IPC 完成 supervisor 重启、boot ID 变化、延迟和断线测试；当前 Unix datagram 已有真实内核套接字的双向、WouldBlock、peer 缺失/来源错误/超长报文、同路径新 boot 重启、路径所有权、State/Event payload、完整命令 target 到生命周期接受测试，并有 ProcBuf 读回到模拟 PDO 目标的端到端软件证据。共享内存、RPMsg、部署 ACL、发布级产品策略生成/冻结、生产延迟/WCET、长时压力和实物 HIL 仍需单独验收。 |
...
R2 ProcBuf 活动命令执行增量：`Cia402AxisCommandPolicy` 由产品配置在激活前冻结每轴有符号位置/速度/转矩比例、位置偏置、SI 机械边界、速度/转矩上限和每周期位置步长。`prepare_cia402_command` 对 ABI-v5 Command page 重做结构、到期时间、轴掩码、模式和完整 permit 身份检查，在局部固定数组内完成最近整数目标量化与向下取整限幅；任何非有限值、错误策略、越界或 raw 溢出均在触碰 guard、Domain、帧池和端口前返回轴定位错误。`StopCycleContext` 的直接、调度和统一生产服务 ProcBuf 入口在执行边界重新比对当前周期 permit，握手阶段不写目标，Switched On 使能边沿写已验证实际反馈，Operation Enabled 后才写期望目标，并复用现有活动帧的可写覆盖、别名、限幅、构帧、TX 和同周期停机回退事务。Linux 模拟端口已覆盖 ProcBuf 发布/读取、permit rearm、SI 换算、实际值保持、期望 PDO 和拒绝不变性；该证据不提供产品策略生成、真实驱动响应、制动/机械适用性、目标 WCET 或安全资格。
