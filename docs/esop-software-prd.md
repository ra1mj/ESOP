...
| R2：伺服与实时资格 | CoE、DC、ProcBuf、MLG、CiA 402 单轴/双轴和 IO、故障策略。 | 两种驱动 + IO 完成 HIL；Q1/Q2、无动态分配、MLG 状态机和故障矩阵证据通过。当前已具备跨层质量门投影、LifecycleSnapshot、双轴独立许可、配置停止动作、新 permit epoch 恢复、ProcBuf v4 生命周期摘要、逐轴请求/成功提交控制字/反馈证明位及原始周期质量位图、固定转换历史发布、有序 lifecycle 转换/逐轴超时事件生成、环满重试及历史覆盖显式确认接口、基于已提交 Domain 的新鲜停机反馈适配器、冻结计划的停机帧提交与跨轴别名拒绝、单 Domain 活动/停机/禁止输出周期分支协调器（真实周期核对、模式/目标限幅及输出 PDO 别名验证、新激活的目标守卫重置并在使能边沿由新鲜实际反馈播种、使能边沿目标与实际反馈一致、成功 TX 后推进目标、活动帧失败同周期撤销许可并尝试停机、失败 TX 仍发布证据、后续新鲜反馈确认、锁存超时后发送 Disable、State 后发布有序事件）、TX/构帧失败后停机证明拒绝与重试及 RX 超时索引复用边界测试、证据绑定能力清单和构建/性能报告校验测试；完整生产周期所有者（最终 deadline/其他 Domain 接线）、受控 Hold/Ramp 与实物 HIL 证据仍待补齐。 |
...
R2 增量进展：生命周期输出分支已支持固定容量的多 Domain 调度质量投影。冻结调度的数量与 ID 顺序、每周期必到的运动 Domain 及其真实质量均在发送前校验；辅助 Domain 非到期样本按周期复用，到期漏收则触发同周期停机与许可撤销。当前其他 Domain 的快照仍由调用者采集和提供；完整生产周期所有者、最终发送后 deadline 判定、其他 Domain 输出调度和实物 HIL 验收尚未完成，上表 R2 出口条件仍未满足。
