...
路由层只校验 key、方向和 payload 类型；它不执行 Protobuf 编解码，也不承担认证。实际 payload 必须先经过生成的 v1 binding 和共享 target 结构验证，再由命令准入层执行 source/authority/TTL/sequence/axis policy，最后从已准入 `MotionPermit` 构造 ProcBuf Command page。
