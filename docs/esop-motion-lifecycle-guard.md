...
当前固定容量 RT 实现支持在构造 `LifecycleGuard` 时通过 `AxisStopPolicy` 冻结最多 32 轴的动作；`cycle_axes` 对同一周期返回受守卫借用约束的逐轴决策，停机轴掩码保持为原已激活 permit 的轴集合，其他轴保持 inhibit。默认 CiA 402 多轴适配器 `step_axis_bank` 对已停机轴分别输出 `QUICK_STOP` 或 `DISABLE`，并继续将 `HOLD`/`RAMP_TO_ZERO` 降级为 `DISABLE`，因此默认生产周期入口的失效安全行为不变。显式 `ControlledStopPlanner` 与 `submit_controlled_stopping_frame` 提供可选受控路径：`HOLD` 仅支持 CSP，并锁定停车序列第一份合格实际位置；`RAMP_TO_ZERO` 仅支持 CSV/CST，并按产品冻结的原始单位每周期步长，从当前合格速度/转矩反馈收敛到零。动作、模式、限幅和 MLG 转换序号在同一停车序列中不可变化；规划器先预演，只有整帧被端口接受后才提交状态和首次发送证据。`ControlledStopCycleState` 已将该能力接入 stop-only、共享 RX、控制服务和统一生产服务周期入口：它由调用方长期持有并冻结限幅，首次缺模式确认、缺反馈、PDO 不完整/别名、周期重放、限幅变化、构帧或 TX 失败会清空规划器，并把当前 MLG 转换序列锁定到默认 `DISABLE`/`QUICK_STOP`，后续周期不得重新启用 Hold/Ramp；新的转换序列才清除该锁存。`StopCycleOutcome` 与 `ScheduledProductionRelease` 显式发布受控路径是否成功使用或已回退，回退帧自身的 TX 结果仍独立报告。原始单位限幅仍需按具体驱动缩放、机械、周期时间和制动策略完成产品冻结。以上均为软件仿真验证，真实驱动、制动器和安全链的 HIL 仍是 FR-042 的待验收项。
...
ProcBuf 活动入口在上述 raw-target 路径之外提供附加的 SI 命令适配，不改变旧 API。`Cia402AxisCommandPolicy` 必须由产品配置在激活前冻结：每轴有符号位置/速度/转矩比例负责方向，raw 位置偏置只用于绝对位置，并同时声明 SI 位置边界、速度/转矩上限和每周期位置步长。`prepare_cia402_command` 要求 Command page 结构有效、未过期、请求运动、模式与所选轴一致，并且从页面重建的完整 permit 与 `LifecycleGuard` 当前 permit 完全相同；目标按最近整数换算，非负 guard 限制向下取整，任何策略错误、机械越界、命令上限越界或 raw 溢出都在触碰目标守卫、Domain、帧池和端口前失败。`run_with_procbuf_command`、调度变体和统一生产服务受控停车变体在输出边界再次核对周期决策持有的 permit，随后由 `submit_prepared_active_frame` 从当前已验证 Statusword 选择握手无目标、使能边沿实际反馈保持或 Operation Enabled 期望目标，再委托同一个 `submit_active_frame` 事务。软件测试证明 ProcBuf 发布/读取到模拟帧字节的因果链，不证明真实驱动已执行，也不生成或认证产品策略。
...
稳定循环使用固定容量 `ScheduledProductionCycleOwner` 保存阶段所有权。首次 priming 后进入 `ReceiveArmed`；只有相同周期和 generation 的真实 Bank RX 完成后才进入 `OutputPending`；生命周期结算必须返回下一周期、按 `u16` wrapping 规则得到的唯一后继 generation、冻结 due mask、预期/已接受帧数，以及与调度器期望绝对值完全一致的 RX deadline，随后直接回到 `ReceiveArmed`，重复 priming 会被拒绝。部分 TX 仍必须进入下一次 RX，使已经武装的索引被消费或超时，不能通过重发覆盖。`ScheduledProductionRelease` 只在 handoff 完整、TX 后与发布后 deadline 均合格、State 和事件均发布成功时允许任务释放；运动被安全停止本身不阻止释放，但发布失败、deadline miss 或过程帧缺失会阻止合格结算。受控入口还把 `controlled_stop_used` / `controlled_stop_fallback` 复制到 release，产品若要求 Hold/Ramp 成功而不接受默认回退，必须把该证据加入自身资格门；调度器仍会按 fail-closed 停机结果完成周期交接。所有者与统一服务调度器的软件闭环不替代目标硬件 WCET 与实物 HIL。

发布侧使用 `qualification/r2_qualification.json` 与 `scripts/validate-r2-qualification.py` 把这一产品资格边界变成 fail-closed 证据门。通过声明必须冻结每轴模式、停止动作和与 `ControlledStopLimits` 同字段的原始单位限幅，并将两厂商驱动、IO、CSP/CSV/CST、配置停止动作、故障矩阵、构建资源、Q1/Q2 性能、已知限制及三类发布审查绑定到路径与 SHA-256；build/performance 报告还必须共享同一 tested commit、config hash 和 HIL topology hash。仓库内基线为未合格，Linux 仿真、结构正确的 JSON 或单独一次通过报告均不能把它升级为产品资格。

启动、映射和 DC 配置等非邮箱服务可使用 `run_dc_and_control_cycle` 接入同一周期。请求句柄仍归原服务 FSM：`Prepared` 最多发送一次，`InFlight` 禁止重发并只等待共享 RX 完成或严格超时，`Complete` / `Failed` 必须由匹配的服务校验动作身份后消费和释放。传输层返回私有构造的发送前/接收后状态证据，并在 DC 或控制发送失败后继续 Domain/DC/control RX 收尾；`ScheduledDomainBank::confirms_control_cycle` 把报告绑定到最新真实周期和 Domain 质量。MLG 调用方还必须用 `ScheduledControlGate` 声明该服务拥有 Platform、Configuration、Topology 或 Drive 门，并在服务 FSM 消费终态后提供真实就绪值；传输或请求失败只能清除该门，不能被调用方重新置真。仿真覆盖真实映射控制器成功推进、丢响应后跨周期不重发、超时故障和稳定生产交接。完整生产入口仍需统一服务选择、恢复和升级策略。

统一生产服务路径使用 `ScheduledProductionServiceScheduler` 和 `ScheduledProductionServiceCycleReport`。调度器以 Startup、PDO Configuration、Mapping、DC Configuration、Mailbox 的固定顺序选择当前服务，并独占一个请求句柄；`Prepared` 在 DC/期限阻止上线时释放并重建，重建前若原动作已过期则不再占用池槽而直接进入所属 FSM 的 Timeout 路径，提交预检拒绝也会释放尚未上线的调度器句柄；`InFlight` 跨周期保持且不重发，终态由所属 FSM 消费后释放。PDO 服务支持原有单配置绑定和固定容量 `PdoConfigBatch`；批模式按产品冻结顺序选择当前控制器、邮箱和 MailboxConfig，持续核对 PDO pending 与邮箱事务身份，仅在精确 upload 回读后自动推进下一 job，整批完成前不会标记就绪。终态邮箱错误锁存为当前 job 的 PDO transport fault 且不推进索引，只有显式批重启才能恢复。长期控制请求保留其原始 generation，RX 仅允许 `ControlRxConsumer` 在相同 InFlight 槽、index 和 generation 仍成立时授权跨周期完成，Domain/DC 的当前周期 generation 检查不放宽。Bank 验证统一报告的底层控制/邮箱证据；报告同时携带 `startup_phase` 和可选批 phase/index/count/current station。`other_cycle_facts_from_production_service_cycle` 和 `StopCycleContext::run_service_cycle_with_outputs_until` 固定将 Startup 归属 Topology、PDO Configuration/Mapping/DC/Mailbox 归属 Configuration，不再接受调用方自报 gate 或 ready。需要产品受控停车时改用 `run_service_cycle_with_controlled_stop_until` 并传入持续存在的 `ControlledStopCycleState`；`ScheduledProductionCycleOwner::complete_service_cycle` 可据此完成稳定循环 RX 阶段交接。启用 Startup 的可选 PREOP 配置屏障后，调度器只会在完整 PDO 批次及所需 Mapping/DC 控制器全部处于真实 `Complete` 阶段时释放启动流程；该软件闭环不替代 MailboxConfig/映射/DC 描述符发现、Hold/Ramp 策略的逐产品资格、目标硬件 WCET 和实物 HIL。
...
邮箱服务可成对使用 `submit_dc_and_mailbox` 与 `receive_with_dc_and_mailbox`。TX 入口只在邮箱动作到期时申请固定池请求，并把返回句柄显式交回调用者跨周期保存；`Prepared` 才发送，已有 `InFlight` 只随本周期 DC 继续等待，禁止重复提交。DC 或期限在控制帧前失败时，入口释放尚未上线的请求槽但保留邮箱动作，下一周期可重新申请；调用者传入的请求若预检失败则不被静默释放。RX 入口接收前校验请求确属当前邮箱动作，复用 Domain/DC/控制共享 RX，然后只在请求完成或失败时消费并释放；尚在途请求不被伪装为成功。控制发送失败使用邮箱原有重试预算和延时，耗尽则故障。需要固定阶段所有权时，可在过程 Domain 帧已提交后调用 `run_dc_and_mailbox_cycle`：它把两端入口收进同一有界调用，服务 TX 一旦通过预检，即使报告 DC/控制发送失败也必须继续共享 RX 收尾，返回值只允许携带仍存活的请求句柄，并额外观测 RX 完成时的期限。RX 阶段若出现本应由同一预检排除的索引/世代/请求绑定错误，错误仍保留 TX 报告，外层必须故障或重新初始化，不能继续运动。调用者仍须检查发送与邮箱结果、接收质量及阶段期限，不能仅根据请求槽释放或上周期 DC lock 放行运动；该阶段期限也不是过程输出、State/事件发布后的最终 deadline。以上入口不负责启动、其他控制服务、过程输出或完整周期调度，仍需 HIL 资格验证。
