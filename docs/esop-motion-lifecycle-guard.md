...
邮箱服务可成对使用 `submit_dc_and_mailbox` 与 `receive_with_dc_and_mailbox`。TX 入口只在邮箱动作到期时申请固定池请求，并把返回句柄显式交回调用者跨周期保存；`Prepared` 才发送，已有 `InFlight` 只随本周期 DC 继续等待，禁止重复提交。DC 或期限在控制帧前失败时，入口释放尚未上线的请求槽但保留邮箱动作，下一周期可重新申请；调用者传入的请求若预检失败则不被静默释放。RX 入口接收前校验请求确属当前邮箱动作，复用 Domain/DC/控制共享 RX，然后只在请求完成或失败时消费并释放；尚在途请求不被伪装为成功。控制发送失败使用邮箱原有重试预算和延时，耗尽则故障。调用者必须原样传递 TX 返回的句柄、检查发送与邮箱结果、接收质量及周期截止时间，不能仅根据请求槽释放或上周期 DC lock 放行运动。该成对入口不负责启动、其他控制服务、过程输出或完整周期调度，仍需 HIL 资格验证。
