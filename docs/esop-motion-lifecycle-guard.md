...
当前固定容量 `LifecycleGuard` 在进入和退出维护模式、锁存故障时撤销旧门槛资格；发生边界转换之前或同周期的门槛上报不得计入新的稳定窗口。维护状态的周期动作显式请求 `Disable`，不得将普通 `Hold` 当作驱动断使能；从运动/停止中进入维护，退出后仍须维持 `Disable` 请求并显式确认停机，不能跳过停止确认。已锁存故障不接受维护开关转换，避免绕过显式恢复。`clear_fault` 只有在故障后的必需门槛重新连续合格时才接受，并再次撤销旧 permit；恢复运动仍需显式 `request_rearm` 和更新 epoch 的 permit。维护退出也须由退出后的新观测重新满足稳定窗口，不能沿用维护前的健康计数。此状态机约束不替代实物驱动停机确认、产品安全链和 HIL 验证。
