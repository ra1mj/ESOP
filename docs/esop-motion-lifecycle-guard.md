...
稳定循环使用固定容量 `ScheduledProductionCycleOwner` 保存阶段所有权。首次 priming 后进入 `ReceiveArmed`；只有相同周期和 generation 的真实 Bank RX 完成后才进入 `OutputPending`；生命周期结算必须返回下一周期、按 `u16` wrapping 规则得到的唯一后继 generation、冻结 due mask、预期/已接受帧数，以及与调度器期望绝对值完全一致的 RX deadline，随后直接回到 `ReceiveArmed`，重复 priming 会被拒绝。部分 TX 仍必须进入下一次 RX，使已经武装的索引被消费或超时，不能通过重发覆盖。`ScheduledProductionRelease` 只在 handoff 完整、TX 后与发布后 deadline 均合格、State 和事件均发布成功时允许任务释放；运动被安全停止本身不阻止释放，但发布失败、deadline miss 或过程帧缺失会阻止合格结算。所有者现可核验并消费完整的非邮箱控制周期，但仍未负责在邮箱、启动、映射与 DC 配置服务之间做生产级选择、请求重建和故障恢复，也不替代目标硬件 WCET 与实物 HIL。
...
统一生产服务路径使用 `ScheduledProductionServiceScheduler` 和 `ScheduledProductionServiceCycleReport`。调度器以 Startup、Mapping、DC Configuration、Mailbox 的固定顺序选择当前服务，并独占一个请求句柄；`Prepared` 在 DC/期限阻止上线时释放并重建，重建前若原动作已过期则不再占用池槽而直接进入所属 FSM 的 Timeout 路径，提交预检拒绝也会释放尚未上线的调度器句柄；`InFlight` 跨周期保持且不重发，终态由所属 FSM 消费后释放。故障服务继续占据优先级，只有外部重新启动该控制器才能恢复选择。长期控制请求保留其原始 generation，RX 仅允许 `ControlRxConsumer` 在相同 InFlight 槽、index 和 generation 仍成立时授权跨周期完成，Domain/DC 的当前周期 generation 检查不放宽。Bank 验证统一报告的底层控制/邮箱证据；`other_cycle_facts_from_production_service_cycle` 和 `StopCycleContext::run_service_cycle_with_outputs_until` 固定将 Startup 归属 Topology、Mapping/DC/Mailbox 归属 Configuration，不再接受调用方自报 gate 或 ready。`ScheduledProductionCycleOwner::complete_service_cycle` 可据此完成稳定循环 RX 阶段交接。该软件闭环仍不替代 Hold/Ramp 策略、目标硬件 WCET 和实物 HIL 资格。
