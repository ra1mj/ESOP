...
统一生产服务路径使用 `ScheduledProductionServiceScheduler` 和 `ScheduledProductionServiceCycleReport`。调度器以 Startup、Mapping、DC Configuration、Mailbox 的固定顺序选择当前服务，并独占一个请求句柄；`Prepared` 在 DC/期限阻止上线时释放并重建，重建前若原动作已过期则不再占用池槽而直接进入所属 FSM 的 Timeout 路径，提交预检拒绝也会释放尚未上线的调度器句柄；`InFlight` 跨周期保持且不重发，终态由所属 FSM 消费后释放。故障服务继续占据优先级，只有外部重新启动该控制器才能恢复选择。长期控制请求保留其原始 generation，RX 仅允许 `ControlRxConsumer` 在相同 InFlight 槽、index 和 generation 仍成立时授权跨周期完成，Domain/DC 的当前周期 generation 检查不放宽。Bank 验证统一报告的底层控制/邮箱证据；`other_cycle_facts_from_production_service_cycle` 和 `StopCycleContext::run_service_cycle_with_outputs_until` 固定将 Startup 归属 Topology、Mapping/DC/Mailbox 归属 Configuration，不再接受调用方自报 gate 或 ready。需要产品受控停车时改用 `run_service_cycle_with_controlled_stop_until` 并传入持续存在的 `ControlledStopCycleState`；`ScheduledProductionCycleOwner::complete_service_cycle` 可据此完成稳定循环 RX 阶段交接。该软件闭环仍不替代 Hold/Ramp 策略的逐产品资格、目标硬件 WCET 和实物 HIL。
