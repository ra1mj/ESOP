...
邮箱服务可改用 `receive_with_dc_and_mailbox`：接收前校验请求确属当前邮箱动作，复用 Domain/DC/控制共享 RX，然后只在请求完成或失败时消费并释放；尚在途或因前置 TX 中断未发出的请求不被伪装为成功。控制发送失败使用邮箱原有重试预算和延时，耗尽则故障；调用者必须检查显式的邮箱结果、接收质量与周期截止时间，不能仅根据请求槽释放或上周期 DC lock 放行运动。该入口不负责启动、其他控制服务或完整周期调度，仍需 HIL 资格验证。
